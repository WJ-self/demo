...
class Trainer(BaseTrainer):
    ...
    Trainer class
    """
    def __init__(self, model, optimizer, config, data_loader, model_recon=None,
    ...
    def forward_sequence(self, sequence, valid=False):
        ...
        for i, item in enumerate(sequence):
            ...
            if valid:
                pred_loss = self.ema_model(image, torch.concat((recon['image'],events),dim=1)) # scalar
            else:
                pred_loss = self.model(image, torch.concat((recon['image'],events),dim=1) # scalar
